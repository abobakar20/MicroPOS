...
export const useSupplierStore = create<SupplierState>((set) => ({
  ...
  addSupplier: (data) => set((state) => ({
    suppliers: [...state.suppliers, { ...data, id: `S${String(state.suppliers.length + 1).padStart(3, '0')}`, totalPurchased: 0, createdAt: new Date().toISOString() }],
  })),
  ...
  addPurchase: (purchaseData) => set((state) => {
    const purchase: Purchase = { ...purchaseData, id: `P${String(state.purchases.length + 1).padStart(3, '0')}`, createdAt: new Date().toISOString() };
    return {
      purchases: [purchase, ...state.purchases],
      suppliers: state.suppliers.map((s) => s.id === purchaseData.supplierId ? { ...s, totalPurchased: s.totalPurchased + purchaseData.totalCost } : s),
    };
  }),
}));
